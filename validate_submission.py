...
def validate_submission(csv_path, strict=True):
    errors = []
    path = Path(csv_path)
    ...
    n = len(data_rows)
    if strict and n != EXPECTED_DATA_ROWS:
        errors.append(
    ...
    for i, cells in enumerate(data_rows):
        ...
        elif strict and not CANDIDATE_ID_PATTERN.match(cid):
            errors.append(
        ...
        try:
            ...
            upper_limit = EXPECTED_DATA_ROWS if strict else n
            if not 1 <= rank <= upper_limit:
                errors.append(f"Row {row_num}: rank must be between 1 and {upper_limit}.")
            elif rank in seen_ranks:
                errors.append(f"Row {row_num}: duplicate rank {rank}.")
            else:
                seen_ranks.add(rank)
        except ValueError:
            errors.append(f"Row {row_num}: rank must be an integer (1–{EXPECTED_DATA_ROWS if strict else n}).")
            rank = None
    ...
    missing = set(range(1, (EXPECTED_DATA_ROWS if strict else n) + 1)) - seen_ranks
    if missing:
        errors.append(
            f"Each rank 1–{EXPECTED_DATA_ROWS if strict else n} must appear exactly once; missing: {sorted(missing)}"
        )
